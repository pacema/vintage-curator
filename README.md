...
Listings are returned in **view order** when you set **`AIRTABLE_LISTINGS_VIEW`** (name and value) in **`.env.local`** for local development.

**On Vercel:** add the same variable in the project’s **Settings → Environment Variables** (`AIRTABLE_LISTINGS_VIEW` with the same `viw…` value as locally), along with `AIRTABLE_API_KEY` and `AIRTABLE_BASE_ID`. Redeploy after changing env vars so the API picks them up.
